...
class _SignScreenState extends State<SignScreen>{
  ...
  final bundle=TextEditingController(),name=TextEditingController(),version=TextEditingController(),build=TextEditingController();

  Future<void> _chooseIpa() async { final v=await importer.pickFiles(extensions:['ipa']); if(v.isEmpty)return; final f=v.first; await store.addFiles([f]); setState(()=>ipaPath=f.path); try{final info=await signing.inspectIpa(f.path); if(!mounted)return; setState((){bundle.text=(info['bundleId']??'').toString();name.text=(info['displayName']??'').toString();version.text=(info['version']??'').toString();build.text=(info['build']??'').toString();});}catch(_){} }
  ...
  Future<void> _sign() async {
    ...
    try{
      ...
      final out=await signing.sign(ipaPath:ipaPath!,p12Path:id.p12Path,p12Password:storedPassword,provisionPath:id.provisionPath,options:SignOptions(bundleId:bundle.text.trim(),displayName:name.text.trim(),version:version.text.trim(),build:build.text.trim(),removeSupportedDevices:removeDevices));
